...
export class Cluster {
    ...
    create(kopsStateStore: string, environment: {}, dependsOn: any[]) {
        ...
        if (this.nodeOpenPorts.length > 0) {
            const getSecurityGroupsCommand = `aws ec2 describe-security-groups --region ${this.awsRegion} --query 'SecurityGroups[?GroupName==\`nodes.${this.name}\`].{ID:GroupId}' --output text`;
            //get node security group id in the region
            let securityGroups = new local.Command(`get security groups ${this.name}`, {
                create: getSecurityGroupsCommand,
                environment: newEnvironment
            }, {dependsOn: [this.kopsValidateCluster]});

            let securityGroupsId = pulumi.all([securityGroups.stdout]).apply(([securityGroupsId]) => {
                            console.log("securityGroupsId: " + securityGroupsId);
